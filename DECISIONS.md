...
## D-012 Dirty Tracking：opt-in 默认不生成，Modify() 统一入口

**背景**
持久化框架需要知道对象何时被修改，以便将变更同步到存储。手动调用 `MakeDirty()` 容易被遗忘。

**为什么是 Modify()，而非其他方案**

| 方案 | 排除原因 |
|------|---------|
| setter 级注入 | 嵌套类型和第三方类型的内部变更不经过外层 setter，无法覆盖；造成"部分字段自动标脏"的混合模式，比全手动更危险，制造虚假安全感 |
| 快照对比（Snapshot diff） | 需要持久化框架侧改造；对象快照的内存开销在游戏服务器规模下未经验证 |
| 事务边界（Begin/Commit） | Commit 可能被遗漏或被 early return 跳过，引入新的遗漏风险 |

Modify() 满足所有约束：覆盖所有变更路径（setter、嵌套类型、第三方类型）；闭包边界保证 MakeDirty 必然被调用；只需 gogen 侧改造，不依赖持久化框架配合。

**决策：opt-in（默认不生成）+ Modify() 统一入口**
- 未显式配置且方法集中无 `MakeDirty()` 时，不生成，维持现有行为
- 仅在以下情况下生成 `Modify(fn func())` 方法：
...
注：早期版本支持字段级 `gogen:"dirty=XXX"` tag，后因复杂度过高已废弃（见 D-015），现为未知选项，linter 报 Error。签名从 `fn func(*T)` 变更为 `fn func()` 的决策见 D-023。
...
**开放方向**（条件成熟时重新评估，属持久化框架侧决策，与 gogen 无关）
- **快照对比**：若框架愿意承担快照内存开销，应用层可完全不写 MakeDirty/Modify。评估要点：只 diff 已加载对象（非全量），内存开销是否可接受。
- **加载即脏**：若业务模式满足"加载必修改"（如玩家进房必有状态变化），框架可在帧结束时保存所有内存中的对象，无需任何标脏操作，代价是额外写入量。
...
```
或通过传入 `Modify()` 的 fn 闭包在调用层做精细化判断——gogen 不越界。
...
## D-023 Modify() 签名：fn func() 而非 fn func(*T)

**背景**
早期生成的 `Modify` 签名为 `fn func(*T)`，实际使用后发现认知问题，变更为 `fn func()`。

**原有签名的问题**

`fn func(*T)` 中，外部变量 `pp` 与闭包参数 `p` 指向同一对象，调用方需在两个名字间选择，产生双名字认知负担。更关键的是：参数 `p` 给人"应该通过 `p` 操作"的暗示，但 Go 无法阻止闭包内忽略 `p` 而直接使用外部的 `pp`——约束力为零，是虚假安全感。

**选择 fn func() 的理由**
1. 消除双名字问题，闭包直接使用外部变量，符合 Go 闭包惯用法
2. 语义诚实：明确表达"你管理变量，我负责 MakeDirty"，不制造虚假约束
3. `fn func(*T)` 中闭包依然可以忽略参数操作错误对象，两种签名的实际约束力对等，均依赖 code review

**fn func() 的已知缺点**
- **无编译器保障**：无法阻止闭包内使用错误变量（多对象作用域下尤其需要注意），与 `fn func(*T)` 约束力对等
- **闭包语法开销**：单字段修改比直接调用 setter 更啰嗦
- **潜在堆分配**：闭包捕获外部变量时可能逃逸到堆，高频修改路径有 GC 压力，实际影响需 profiling 确认
- **嵌套调用重复标脏**：Modify 内再调用 Modify，MakeDirty 会执行多次；若 MakeDirty 是幂等轻量操作（如设置 bool 标志）则无实际影响
