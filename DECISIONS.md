...
**幂等检查**
Set 类方法（SetField/SetFieldAt）：若字段类型可比较（`IsComparable=true`），且有 dirty 方法，则生成幂等检查 `if current == new { return }` 再赋值，避免无效 dirty 通知。集合写方法（Append/Delete/SetVal/DeleteKey）不做幂等检查（语义上必须更新）。
