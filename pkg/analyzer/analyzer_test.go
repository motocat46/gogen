...
func TestPromotedMethods(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.structName, func(t *testing.T) {
			...
			require.True(t, ok, "未找到结构体 %s（已加载的结构体: %v）", tc.structName, structNames(structs))

			// 验证提升方法集合包含预期项
			for _, method := range tc.expectPromoted {
				assert.True(t, sd.PromotedMethods[method],
					"%s.PromotedMethods 应包含 %q，实际内容: %v", tc.structName, method, sd.PromotedMethods)
				// 生成应被阻止
				assert.False(t, sd.CanGenerateMethod(method),
					"%s.CanGenerateMethod(%q) 应返回 false（方法已被提升）", tc.structName, method)
			}

			// 验证非冲突方法不在提升集合中，且可正常生成
			for _, method := range tc.expectNotPromoted {
				assert.False(t, sd.PromotedMethods[method],
					"%s.PromotedMethods 不应包含 %q（误报），实际内容: %v", tc.structName, method, sd.PromotedMethods)
				// 非冲突方法应该可以生成（除非与字段名或手写方法冲突，此处测试用例无此情况）
				assert.True(t, sd.CanGenerateMethod(method),
					"%s.CanGenerateMethod(%q) 应返回 true（ManualMethods=%v, FieldNames=%v）",
...
func TestAnalyze_ExcludePaths(t *testing.T) {
	dir := testdataDir(t)
	pkgs, err := loader.Load(dir, loader.Config{}, ".")
	require.NoError(t, err, "加载包失败")

	// 使用 testdata/examples 目录本身作为排除路径
	structs, err := analyzer.Analyze(pkgs, analyzer.Config{
		ExcludePaths: []string{dir},
	})
	require.NoError(t, err, "Analyze 失败")
	...
	if len(structs) != 0 {
		...
		assert.Empty(t, structs, "排除目录后期望 0 个结构体，实际得到 %d 个: %v", len(structs), names)
	}
}
...
func TestAnalyze_FileFilter(t *testing.T) {
	...
	assert.NotEmpty(t, structs, "FileFilter 后结果为空，期望包含 embed_cases.go 中的结构体")
	for _, s := range structs {
		assert.Equal(t, dir, s.Dir, "结构体 %q 的 Dir 不是 testdata/examples，FilterSet 可能未生效", s.Name)
	}

	// 验证：types.go 中定义的结构体（AllTypes）不出现在结果中
	for _, s := range structs {
		assert.NotEqual(t, "AllTypes", s.Name, "AllTypes 定义在 types.go，FileFilter 过滤后不应出现")
	}
}
...
func loadEdge(t *testing.T) map[string]*model.StructDef {
	...
	dir := edgeDir(t)
	pkgs, err := loader.Load(dir, loader.Config{}, ".")
	require.NoError(t, err, "加载 testdata/analyzer_edge 失败")
	structs, err := analyzer.Analyze(pkgs, analyzer.Config{})
	require.NoError(t, err, "分析 testdata/analyzer_edge 失败")
	m := make(map[string]*model.StructDef, len(structs))
	for _, s := range structs {
		m[s.Name] = s
	}
...
func TestAnalyzeFields_UnexportedFieldsSkipped(t *testing.T) {
	...
	require.True(t, ok, "未找到 WithUnexported 结构体，已加载: %v", structNames(structs))

	active := sd.ActiveFields()
	for _, f := range active {
		assert.NotEqual(t, "id", f.Name, "非导出字段 'id' 不应出现在 ActiveFields 中")
		assert.NotEqual(t, "secret", f.Name, "非导出字段 'secret' 不应出现在 ActiveFields 中")
	}
	...
	require.Len(t, active, len(wantExported),
		"ActiveFields 长度不符（got %v）", exportedNames)
	for _, f := range active {
		assert.True(t, wantExported[f.Name], "意外的字段 %q 出现在 ActiveFields 中", f.Name)
	}
}
...
func TestAnalyzeFields_UnknownTagOptions(t *testing.T) {
	...
	require.True(t, ok, "未找到 UnknownTagOption 结构体，已加载: %v", structNames(structs))
	...
	if len(active) != 2 {
		...
		assert.Len(t, active, 2, "UnknownTagOption 应有 2 个活跃字段，got %v", names)
	}
}
