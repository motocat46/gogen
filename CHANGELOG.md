...
- **单文件模式误删其他 `_access.go`**：指定具体 `.go` 文件生成时，孤儿清理会把同目录其他文件对应的 `_access.go` 误判为孤儿删除。现在单文件模式下跳过孤儿清理，需要清理孤儿时改用包模式（`./...`）。

### 新特性

- **版本号自动读取**：`go install github.com/motocat46/gogen@vX.Y.Z` 安装后，`gogen version` 自动显示正确版本号，无需构建时手动注入 `-ldflags`。
